...
//	private static List<String> listStrings1 = new ArrayList<String>();
...
//		    			   Toast.LENGTH_SHORT).show();
		...
		OnItemDragNDropListener listener = new OnItemDragNDropListener() 
		{
			...
			{
				if(mHighlightList.get(position))
...
//    	            mDbHelper.update(id, strNote,1);
    	            mDbHelper.update(position+1, strNote,1);
                }
                else
                {
    	            image.setImageResource(android.R.drawable.checkbox_off_background);
//    	            mDbHelper.update(id, strNote,0);
    	            mDbHelper.update(position+1, strNote,0);
                }
...
//                System.out.println("markingIndex = " + markingIndex);
                ...
                if( cursor.getLong(markingIndex) == 1)                
                {
                	...
                	tv.setTextColor(Color.rgb(64, 92, 64));
                }
                else
                {
...
//            	tv.setBackgroundColor(Color.rgb(10, 32, 10));
	...
	public void addNewItem() 
	{
		...
		if (bMove)
		{
			...
			System.out.println("last id check:" + mNotesCursor.getString(mColumnIndex));
			mNoteNumber1 = Integer.parseInt(mNotesCursor.getString(mColumnIndex)) + 1; 
		} else {
			//first
			mNoteNumber1 = mNotesCursor.getCount()+1;
		}

		String noteName = "";
//		String noteName = "(" + (mNoteNumber1) + ")";
//		mDbHelper.insert(noteName);
//		fillData();
//		listStrings1.add(noteName);
...
//							mDbHelper.update(mNoteNumber1, editString1, 0);
//							listStrings1.set(listStrings1.size()-1, editString1);//list由0算起
						}
						fillData();
					}
				}).show();
...
//		v.setBackgroundColor(Color.BLACK);
    ...
    static final int CONFIG_ID = Menu.FIRST+1;
	
    @Override public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        menu.add(Menu.NONE, ADD_NEW_ID, 0, "[Add New Note]")
        	.setIcon(R.drawable.add_new)
            .setShowAsAction(MenuItem.SHOW_AS_ACTION_ALWAYS);
        menu.add(Menu.NONE, CONFIG_ID, 1, "[Config]")
